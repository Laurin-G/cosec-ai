...
/*
 * Validate that a PKCS#11 provider path is safe to load.
 * Only absolute paths outside of /tmp and world-writable directories
 * are accepted, to prevent privilege escalation (CVE-2016-10009).
 */
static int
pkcs11_provider_allowed(const char *path)
{
	if (path == NULL || path[0] != '/')
		return 0;
	/* Reject paths under /tmp or /var/tmp */
...
process_add(void)
{
	...
	pin = get_string(NULL);
	if (!pkcs11_provider_allowed(name)) {
		error("refusing PKCS#11 provider \"%s\": not an allowed path",
		    name ? name : "(null)");
